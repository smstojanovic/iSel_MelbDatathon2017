...
tat %>%
  ...
  mutate(pwdiff = prescription_week - lag(prescription_week)) %>% 
  View
```
...
tat %>% 
  select(Patient_ID,
         ...
         atc3,
         ATCLevel5Name) %>% 
  arrange(Patient_ID, prescription_week, dispense_week) %>% 
  View
...
repeat_numbers %>% 
  ...
  group_by(NHS_Code) %>% 
  mutate(frac = n / sum(n)) %>% 
  ggplot(aes(y = NHS_Code, x = RepeatsTotal_Qty, fill = frac)) +
  geom_tile() +
  scale_x_discrete()
```
...
```{r}
tat %>% 
  count(Prescriber_ID, ATCLevel5Name) %>% 
  ungroup %>% 
  filter(n < 25) %>% 
  ggplot(aes(x = n, colour = ATCLevel5Name)) +
  geom_density()
```
